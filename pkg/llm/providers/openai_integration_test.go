...
// TestOpenAIComplete_RealAPI tests a real completion call to OpenAI.
// This is a Tier 2 test: single real API call with cost tracking.
func TestOpenAIComplete_RealAPI(t *testing.T) {
	integration.SkipWithoutEnv(t, "OPENAI_API_KEY")
	...
	provider, err := NewOpenAIProvider(cfg.OpenAIAPIKey)
	if err != nil {
	...
	modelInfo := llm.GetModelByID(capabilities.Models, resp.Model)
	if modelInfo == nil {
...
// TestOpenAIStream_RealAPI tests real streaming completion from OpenAI.
// This is a Tier 2 test: single streaming call with token counting.
func TestOpenAIStream_RealAPI(t *testing.T) {
	integration.SkipWithoutEnv(t, "OPENAI_API_KEY")
	...
	for chunk := range chunks {
		...
		if chunk.RequestID != "" && model == "" {
			// Will extract model from chunks if available
			model = "gpt-3.5-turbo" // fast tier default
		}
	}
	...
	if finalUsage == nil {
		...
		// Track cost
		capabilities := provider.Capabilities()
		modelInfo := llm.GetModelByID(capabilities.Models, model)
		if modelInfo == nil {
...
// TestOpenAIToolCalling_RealAPI tests tool calling with real OpenAI API.
// This is a Tier 2 test: single tool calling round-trip.
func TestOpenAIToolCalling_RealAPI(t *testing.T) {
	integration.SkipWithoutEnv(t, "OPENAI_API_KEY")
	...
	if resp.FinishReason == llm.FinishReasonToolCalls {
		...
		// Model may respond directly without tool use
...
// TestOpenAIErrorHandling_RealAPI tests error handling with real API.
func TestOpenAIErrorHandling_RealAPI(t *testing.T) {
	t.Run("Invalid API Key", func(t *testing.T) {
		integration.SkipWithoutEnv(t, "OPENAI_API_KEY")

		// Create provider with invalid key
		provider, err := NewOpenAIProvider("sk-invalid-key")
		if err != nil {
			t.Fatalf("Failed to create provider: %v", err)
		}
		...
		// This should fail with 401 and NOT retry
		_, err = provider.Complete(ctx, req)
		if err == nil {
			t.Error("Expected authentication error but got success")
		}

		// Verify error is permanent
