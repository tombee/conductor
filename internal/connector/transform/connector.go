...
//
// The transform connector does NOT import the connector package to avoid import cycles.
// The builtin registry bridges between transform operations and the Connector interface.
package transform
...
// TransformConnector implements the Connector interface for transform operations.
type TransformConnector struct {
	config *Config
}

// Config holds configuration for the transform connector.
type Config struct {
...
// DefaultConfig returns sensible defaults for transform connector configuration.
...
// New creates a new transform connector instance.
