...
func NewCloudWatchConnector(config *api.ConnectorConfig) (connector.Connector, error) {
	...
	if config.Transport.Name() != "aws_sigv4" {
		return nil, fmt.Errorf("CloudWatch connector requires aws_sigv4 transport, got: %s", config.Transport.Name())
	}
...
func (c *CloudWatchConnector) Name() string {
	return c.name
}
...
func (c *CloudWatchConnector) Operations() []api.OperationInfo {
...
func (c *CloudWatchConnector) OperationSchema(operation string) *api.OperationSchema {
...
func (c *CloudWatchConnector) getSequenceToken(logGroup, logStream string) string {
	c.tokenMutex.RLock()
...
func (c *CloudWatchConnector) setSequenceToken(logGroup, logStream, token string) {
	c.tokenMutex.Lock()
...
func (c *CloudWatchConnector) clearSequenceToken(logGroup, logStream string) {
	c.tokenMutex.Lock()
	defer c.tokenMutex.Unlock()
