...
func TestNewGitHubIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			conn, err := NewGitHubIntegration(tt.config)
			if (err != nil) != tt.wantError {
				t.Errorf("NewGitHubIntegration() error = %v, wantError %v", err, tt.wantError)
				return
			}
...
func TestGitHubIntegration_Operations(t *testing.T) {
...
func TestGitHubIntegration_ListIssues(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Verify request path
		expectedPath := "/repos/test-owner/test-repo/issues"
		if r.URL.Path != expectedPath {
...
func TestGitHubIntegration_CreatePR(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Verify request path
...
func TestGitHubIntegration_ExecutePaginated(t *testing.T) {
	page := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		page++
...
func TestGitHubIntegration_ExecutePaginatedWithoutPagination(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		json.NewEncoder(w).Encode([]Issue{
			{Number: 1, Title: "Issue 1", State: "open"},
...
func TestGitHubIntegration_ErrorHandling_Validation(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestGitHubIntegration_ErrorHandling_RateLimit(t *testing.T) {
...
func TestGitHubIntegration_ErrorHandling_Auth(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestGitHubIntegration_UnknownOperation(t *testing.T) {
	httpTransport, err := transport.NewHTTPTransport(&transport.HTTPTransportConfig{
		BaseURL: "https://api.github.com",
	})
	if err != nil {
...
func TestGitHubIntegration_UnsupportedPaginatedOperation(t *testing.T) {
	httpTransport, err := transport.NewHTTPTransport(&transport.HTTPTransportConfig{
		BaseURL: "https://api.github.com",
	})
	if err != nil {
