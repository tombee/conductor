...
// sendEvent sends a structured event to Splunk HEC.
func (s *SplunkIntegration) sendEvent(ctx context.Context, inputs map[string]interface{}) (*operation.Result, error) {
	// Validate required parameter: fields
	fields, ok := inputs["fields"].(map[string]interface{})
	if !ok || len(fields) == 0 {
		return nil, fmt.Errorf("missing required parameter: fields")
	}

	// Build HEC event payload with explicit fields
