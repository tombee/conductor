...
func TestNewJenkinsIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := NewJenkinsIntegration(tt.config)
			if (err != nil) != tt.wantError {
				t.Errorf("NewJenkinsIntegration() error = %v, wantError %v", err, tt.wantError)
			}
		})
	}
}
...
func TestJenkinsIntegration_Operations(t *testing.T) {
	...
	conn, err := NewJenkinsIntegration(config)
	if err != nil {
		t.Fatalf("NewJenkinsIntegration() error = %v", err)
	}

	jc := conn.(*JenkinsIntegration)
	ops := jc.Operations()
...
func TestJenkinsIntegration_BasicAuth(t *testing.T) {
...
func TestJenkinsIntegration_GetBuild(t *testing.T) {
...
func TestJenkinsIntegration_ListJobs(t *testing.T) {
	// Create a test server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
...
func TestJenkinsIntegration_ErrorHandling(t *testing.T) {
...
func TestJenkinsIntegration_UnknownOperation(t *testing.T) {
	httpTransport, err := transport.NewHTTPTransport(&transport.HTTPTransportConfig{
		BaseURL: "https://jenkins.example.com",
	})
	if err != nil {
