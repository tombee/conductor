...
// It queries the backend for historical runs and merges with in-memory active runs.
func (s *StateManager) ListRuns(filter ListFilter) []*RunSnapshot {
	s.mu.RLock()
	defer s.mu.RUnlock()

	// Track runs by ID to avoid duplicates (in-memory takes precedence)
	seenIDs := make(map[string]bool)
	var result []*RunSnapshot

	// First, add in-memory runs (active runs have the most up-to-date state)
	...
	// Then, query backend for historical runs (if backend supports listing)
	if lister, ok := s.backend.(backend.RunLister); ok {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()

		beFilter := backend.RunFilter{
			Status:   string(filter.Status),
			Workflow: filter.Workflow,
		}
		backendRuns, err := lister.ListRuns(ctx, beFilter)
		if err == nil {
			for _, beRun := range backendRuns {
				// Skip if already in memory (in-memory state is more current)
				if seenIDs[beRun.ID] {
					continue
				}
				// Convert backend run to snapshot
				result = append(result, s.backendRunToSnapshot(beRun))
			}
		}
	}
...
// backendRunToSnapshot converts a backend.Run to a RunSnapshot.
// Used when loading historical runs from the backend.
func (s *StateManager) backendRunToSnapshot(beRun *backend.Run) *RunSnapshot {
	// Parse status string to RunStatus
	status := RunStatus(beRun.Status)

	// Create progress from backend fields
	var progress *Progress
	if beRun.CurrentStep != "" || beRun.Completed > 0 || beRun.Total > 0 {
		progress = &Progress{
			CurrentStep: beRun.CurrentStep,
			Completed:   beRun.Completed,
			Total:       beRun.Total,
		}
	}
	...
	return &RunSnapshot{
		ID:            beRun.ID,
		WorkflowID:    beRun.WorkflowID,
		Workflow:      beRun.Workflow,
		Status:        status,
		CorrelationID: beRun.CorrelationID,
		Inputs:        beRun.Inputs,
		Output:        beRun.Output,
		Error:         beRun.Error,
		Progress:      progress,
		StartedAt:     beRun.StartedAt,
		CompletedAt:   beRun.CompletedAt,
		CreatedAt:     beRun.CreatedAt,
		// Note: Fields like SourceURL, Workspace, Profile, Provider, Model,
		// Timeout, Security, AllowHosts, AllowPaths, MCPDev are not persisted
		// in the backend, so they will be empty for historical runs.
	}
}
