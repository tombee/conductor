...
This document provides comprehensive API documentation for all public packages in Conduct. These packages are designed to be embedded in Go applications.
...
```go
// Create store
store := workflow.NewSQLiteStore("conduct.db")
...
Adapt Conduct's LLM provider to agent's interface:
...
// Conduct's LLM provider
conductProvider := llm.GetDefault()

// Create adapter for agent
agentProvider := agent.NewLLMAdapter(conductProvider)
...
outputs, _ := registry.Execute(ctx, "http", map[string]interface{}{
    "method": "GET",
    "url":    "https://api.github.com/repos/tombee/conduct",
