...
This guide provides procedures for operators to diagnose and resolve common connector issues.

## Runbook: Connector Health Check

Test connectivity and auth for all configured connectors.

```bash
# Test a specific connector
conductor connector test github
...
conductor connector status --rate-limits
...
3. **Long-term fix**
   Update connector config to reduce request rate:
   ```yaml
   connectors:
     github:
...
## Runbook: Debugging Failed Connector Steps

**Symptoms**: Connector step returns error in workflow execution

**Diagnosis Steps:**

1. **Check error type** in workflow output
   ```
   Error: connector step failed
...
4. **Test operation in isolation**
   ```bash
   conductor connector invoke github.create_issue \
...
3. **Test connectivity**
   ```bash
   conductor connector test internal_api
   ```
...
**Symptoms**: Connector steps taking longer than usual
...
## Runbook: Connector Configuration Validation

**Before deploying** new connector configurations:
...
3. **Test with minimal inputs**
   ```bash
   conductor connector invoke my_api.health_check
   ```
...
### Circuit Breaker: Disable Failing Connector

If a connector is causing cascading failures:
...
2. **Temporary disable** (if supported by your deployment)
   ...
   - id: use_connector
     type: connector
     connector: failing_connector.operation
