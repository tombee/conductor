...
This guide shows you how to create custom connectors for any REST API in your workflows.
...
connectors:
  my_api:
    ...
    operations:                        # Required for inline connectors
...
steps:
  ...
  - id: create_ticket
    type: connector
    connector: helpdesk.create_ticket
    inputs:
...
```bash
conductor connector invoke my_api.get_user --inputs '{"user_id":"123"}'
```
...
- Review [bundled connector examples](./github.md)
- Learn about [MCP vs Connectors](./index.md#when-to-use-connectors-vs-mcp)
