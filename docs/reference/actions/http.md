...
The `http` action provides secure HTTP client capabilities for making API requests from workflows.

## Overview

The HTTP action is a **builtin action** - it requires no configuration and is always available. It provides secure HTTP/HTTPS requests with built-in protections against SSRF, DNS rebinding, and other network-based attacks.
...
By default, the HTTP action **blocks requests to private IP ranges** to prevent Server-Side Request Forgery (SSRF) attacks:
...
The HTTP action caches DNS resolutions to prevent DNS rebinding attacks:
...
The HTTP action limits and validates redirects:
...
- [File Action](file.md) - Filesystem operations
- [Shell Action](shell.md) - Execute shell commands
...
- [GitHub Action](github.md) - Specialized GitHub operations
- [Slack Action](slack.md) - Specialized Slack operations
