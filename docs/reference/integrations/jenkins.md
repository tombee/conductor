...
The Jenkins integration provides integration with Jenkins CI/CD.
...
- id: start_build
  type: integration
  integration: jenkins.trigger_build
  inputs:
    job: my-project/main
    parameters:
      BRANCH: main
...
- id: fetch_logs
  type: integration
  integration: jenkins.get_build_log
  inputs:
    job: my-project/main
    build_number: 42
    start: 0
...
- id: stop_build
  type: integration
  integration: jenkins.cancel_build
  inputs:
    job: my-project/main
    build_number: 42
```
...
- id: fetch_tests
  type: integration
  integration: jenkins.get_test_report
  inputs:
    job: my-project/main
    build_number: 42
```
...
- id: check_queue
  type: integration
  integration: jenkins.get_queue_item
  inputs:
    queue_id: 12345
```
...
- id: get_agents
  type: integration
  integration: jenkins.list_nodes
```
...
3. Increase timeout in integration config
